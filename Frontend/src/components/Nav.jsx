...
function Nav() {
  const { userData, currentCity, currentAddress, cartItems } = useSelector(
    (state) => state.user,
  );
  ...
  const locationLabel = currentAddress || currentCity || "Set location";
  const savedLocation =
  ...
  return (
    <div className="w-full h-[80px] flex items-center justify-between md:justify-center gap-[30px] px-[20px] fixed top-0 z-[9999] bg-[#fff9f6] overflow-visible">
      <LocationPickerModal
        ...
        initialAddress={currentAddress || currentCity || ""}
        initialMode={userData?.locationMode || "auto"}
        onClose={() => setShowLocationPicker(false)}
        onSave={async ({ lat, lon, locationMode }) => {
          const result = await axios.post(
            `${serverUrl}/api/user/update-location`,
            { lat, lon, locationMode },
            { withCredentials: true },
          );
      ...
      {showSearch && userData.role == "user" && (
        <div className="w-[90%] h-[70px]  bg-white shadow-xl rounded-lg items-center gap-[20px] flex fixed top-[80px] left-[5%] md:hidden">
          <div className="flex items-center w-[34%] overflow-hidden gap-[10px] px-[10px] border-r-[2px] border-gray-200">
      ...
      <h1 className="text-3xl font-bold mb-2 text-[#ff4d2d]">Zatpat</h1>
      {userData.role !== "deliveryBoy" && (
        <button
          type="button"
          className="hidden md:flex items-center gap-2 px-3 py-2 rounded-full bg-[#ff4d2d]/10 text-[#ff4d2d] font-medium shadow-sm"
          onClick={() => setShowLocationPicker(true)}
        >
          <FaLocationDot size={18} />
          <span className="max-w-[180px] truncate">Update location</span>
        </button>
      )}
      {userData.role !== "deliveryBoy" && (
        <button
          type="button"
          className="md:hidden flex items-center gap-2 px-3 py-2 rounded-full bg-[#ff4d2d]/10 text-[#ff4d2d] font-medium shadow-sm"
          onClick={() => setShowLocationPicker(true)}
        >
          <FaLocationDot size={18} />
        </button>
